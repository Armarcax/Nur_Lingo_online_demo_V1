...
const needsDownload = items.filter(item => {
  ...
  if (!fs.existsSync(filePath)) return true; // բացակայում է
  const stats = fs.statSync(filePath);
  if (stats.size < 1000) return true; // շատ փոքր է, հավանաբար վնասված
  // Ստուգել, արդյոք HTML չէ
...
const downloadFromGoogleTTS = (text, id) => {
  return new Promise((resolve) => {
    ...
    // Եթե ֆայլն արդեն կա (միգուցե ավելի վաղ ստեղծվել է այս սեսիայում), բաց թողնել
    ...
    const request = https.get(url, (response) => {
      ...
      file.on('finish', () => {
        ...
        // Ստուգել HTML-ի համար (կարող է լինել մերժման էջ)
        const content = fs.readFileSync(filePath, 'utf8');
        if (content.includes('<!DOCTYPE') || content.includes('<html>')) {
          console.log(`⚠️ ${id} (${text}) – HTML է, ոչ MP3, ջնջվում է`);
          fs.unlinkSync(filePath);
          resolve();
          return;
        }
...
const downloadAll = async () => {
  ...
  console.log('⏳ Յուրաքանչյուր ֆայլի համար ~2-3 վայրկյան:');
