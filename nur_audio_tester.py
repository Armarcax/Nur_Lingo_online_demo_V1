...
def test_audio_counts():
    ...
    for rel, (label, min_count) in audio_dirs.items():
        ...
        count = len(mp3s)
        # Check for corrupt (too small) files
        ...
        # Check numbering consistency (no gaps in first 100)
...
def test_manifests():
    ...
    for mpath, (lang, audio_rel) in manifests.items():
        ...
        # Check each entry has a valid audio ID that exists
        missing_files = []
        empty_entries = []
        for key, val in list(mapping.items())[:500]:  # Check first 500
...
def test_audio_manager():
    ...
    checks = {
        "HEAD check before play": ["HEAD", r"method.*HEAD", r"fetch.*HEAD"],
        "MP3 fallback to TTS": ["fallback", "BrowserTTS", "speechSynthesis"],
        "Error handling (no throw)": ["catch", r"try \{", "try{"],
        "audioId usage": ["audioId", r"options\.id", r"opts\.id"],
        "padStart 6 digits": [r"padStart\(6", r"padStart\(\s*6", r"\.padStart"],
...
def test_path_consistency():
    ...
    # Read AudioProviders or AudioManager to find how URL is built
    for fname in ["AudioProviders.ts", "AudioManager.ts", "AudioManifest.ts"]:
        ...
        if urls:
            ...
            if any("/audio/${lang}/${" in u or "/audio/offline/" in u for u in urls):
    ...
    # Check OfflineAudioEngine paths
...
def test_learn_page():
    ...
    checks = {
        "Uses useAudio or useAudioManager": ["useAudio", "AudioManager", "audioManager"],
        "Does NOT use speechSynthesis directly": ["speechSynthesis"],
        "Passes audioId to play": ["audioId", "entry.audioId", "word.audioId"],
        "Has Suspense wrapper (required for useSearchParams)": ["Suspense"],
        "Has error boundary / try-catch": ["catch", "try {"],
    }
...
def test_dictionary_page():
    ...
    checks = {
        "Play button per language (hy/en/ru)": [r"play.*hy", r"play.*en", r"play.*ru", r"🔊"],
        "Back to top button": ["scrollTo", "scrollIntoView", "top.*btn", "↑"],
        "Search/filter": ["search", "filter", "query"],
        "Loads from manifest or API": ["manifest", "/api/lexicon", "fetch"],
        "Audio via AudioService or hook": ["AudioService", "useAudio", "useAudioManager"],
    }
...
def test_audio_scripts():
    ...
    for path, name in scripts.items():
        ...
        # Check for known bug: email without quotes
...
def test_typescript():
    ...
    # Try running tsc --noEmit (if available)
    import subprocess
    try:
