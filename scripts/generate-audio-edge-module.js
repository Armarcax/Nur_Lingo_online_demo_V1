// scripts/generate-audio-final.js
const fs = require('fs');
...
// ─── edge-tts-ի executable-ի ուղին ──────────────────────────────
const edgeBin = path.join(__dirname, '../node_modules/.bin/edge-tts');
// Windows-ում ավելացնում ենք .cmd
const edgeCmd = process.platform === 'win32' ? edgeBin + '.cmd' : edgeBin;

// Ստուգել, արդյոք գոյություն ունի
if (!fs.existsSync(edgeCmd)) {
  console.error(`❌ edge-tts executable չի գտնվել: ${edgeCmd}`);
  console.error('📋 Տեղադրեք edge-tts-ը global-ով `npm install -g edge-tts` կամ ստուգեք node_modules/.bin պանակը:');
  process.exit(1);
}

const generateWithEdgeTTS = async (text, id) => {
  const filePath = path.join(audioDir, `${id}.mp3`);
  // Պաշտպանվել չակերտներից
...
const downloadAll = async () => {
  ...
  for (const item of needsDownload) {
    ...
    // Փոքր դադար՝ ծանրաբեռնվածությունից խուսափելու համար
  ...
  if (finalCheck.length === 0) {
    ...
    console.log('📋 Փորձիր գործարկել սկրիպտը կրկին (այն կփորձի միայն բացակայողները):');
